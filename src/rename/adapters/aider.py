...
project directory. So we can:
  ...
  * Surface Aider sessions in ``rename list`` / ``rename search`` /
    ``rename stats`` (one "session" per project directory that has an
...
the title sticks in ``rename list`` between scans, even though Aider
itself ignores it.
...
class AiderAdapter(Adapter):
    ...
    def set_title(self, session: Session, title: str) -> None:
        ...
        # name sticks in `rename list` across scans.
