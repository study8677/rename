...
def _migrate_legacy(base: Path) -> None:
    """One-time move from the old `retitle` directory to the new `rename`
    one. Runs lazily on first access — if there's no legacy dir or the new
    one is already populated, this is a no-op."""
    ...
    if legacy.exists() and not new.exists():
        try:
            ...
            pass  # Best-effort. Worst case: user keeps the old data; we
                  # start fresh under the new name. They can `mv` by hand.
...
def state_dir() -> Path:
    base = _base("XDG_STATE_HOME", "~/.local/state")
    _migrate_legacy(base)
    return base / APP
