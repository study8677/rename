...
///
/// We're careful about how often we call `rename list / stats` because each
...
/// behind macOS TCC. Without Full Disk Access for Rename.app, every single
...
final class AppState: ObservableObject {
    @Published var cli: RenameCLI?
    @Published var status: RenameStatus?
    @Published var sessions: [SessionPlan] = []
    @Published var stats: RenameStats?
    ...
    /// Cheap: only `rename status --json`. Does NOT scan session stores.
