...
// JSON shapes mirroring `rename <cmd> --json` output.
...
struct RenameStatus: Codable, Hashable {
    let version: String
...
/// One row from `rename list --json`. Fields mirror the Python output 1:1.
...
struct RenameStats: Codable, Hashable {
    let scopeDays: Int?
    let tools: [ToolStats]
    let total: StatTotal
