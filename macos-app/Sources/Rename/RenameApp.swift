...
final class RenameAppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func shouldAutoOpenDashboard() -> Bool {
        if ProcessInfo.processInfo.environment["RENAME_AUTO_OPEN_DASHBOARD"] == "1" {
            return true
        }
...
struct RenameApp: App {
    @NSApplicationDelegateAdaptor(RenameAppDelegate.self) var appDelegate
    ...
    var body: some Scene {
        ...
        MenuBarExtra {
            ...
            .onReceive(
                NotificationCenter.default.publisher(
                    for: RenameAppDelegate.openDashboardOnLaunch
                )
            ) { _ in
                RenameAppDelegate.bringDashboardForward()
        ...
        Window("Rename", id: "dashboard") {
            DashboardView()
                ...
                .onDisappear {
                    // Dashboard closed — drop the Dock icon so rename
                    // stays a pure menu-bar app between dashboard sessions.
                    RenameAppDelegate.restoreMenuBarOnly()
                }
        }
