...
/// Thin wrapper around the `rename` CLI installed elsewhere on the user's
/// machine. The Swift app is a viewer/controller — all real work happens in
/// the Python CLI we invoke here.
enum RenameCLIError: Error, LocalizedError {
    case notFound
    ...
    var errorDescription: String? {
        switch self {
        case .notFound:
            return NSLocalizedString("rename_not_found", comment: "")
        case .failed(let stderr, let code):
            let trimmed = stderr.trimmingCharacters(in: .whitespacesAndNewlines)
            return String(
                format: NSLocalizedString("rename_command_failed_%d_%@", comment: ""),
                code, trimmed
            )
        case .decodeFailed(let msg):
            return String(format: NSLocalizedString("rename_decode_failed_%@", comment: ""), msg)
        }
    }
}

struct RenameCLI {
    /// Resolved path to the `rename` executable on disk. ``nil`` means we
    ...
    static func locate() -> RenameCLI? {
        let candidates = [
            ProcessInfo.processInfo.environment["RENAME_BIN"],
            Self.whichRename(),
            ("~/.local/bin/rename" as NSString).expandingTildeInPath,
            "/opt/homebrew/bin/rename",
        ...
        for raw in candidates {
            ...
            if FileManager.default.isExecutableFile(atPath: url.path) {
                return RenameCLI(executable: url)
            }
        }
        return nil
    }

    private static func whichRename() -> String? {
        let p = Process()
        p.executableURL = URL(fileURLWithPath: "/bin/sh")
        p.arguments = ["-l", "-c", "command -v rename"]
        let out = Pipe()
    ...
    func run(_ args: [String], timeout: TimeInterval = 60) throws -> Data {
        ...
        if p.terminationStatus != 0 {
            ...
            throw RenameCLIError.failed(stderr: stderr, code: p.terminationStatus)
        }
        return stdoutData
    }
    ...
    func status() throws -> RenameStatus {
        let data = try run(["status", "--json"], timeout: 30)
        return try decode(RenameStatus.self, from: data)
    }
    ...
    func stats() throws -> RenameStats {
        let data = try run(["stats", "--json"], timeout: 60)
        return try decode(RenameStats.self, from: data)
    }
    ...
    /// Synchronously trigger `rename once --session <id>`. Blocks until the
    ...
    private func decode<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
        do {
            ...
            throw RenameCLIError.decodeFailed("\(error.localizedDescription) — got: \(preview)")
        }
    }
}

/// Search result shape (from `rename search --json`).
