...
**装上就用，不会偷偷改你的历史。** 第一次跑的时候 retitle 会记一个"基线时间戳"，
之后后台只会改"基线之后才活跃"的会话——你装 retitle 之前的旧聊天不会被自动碰，
除非你明确同意。
...
```bash
retitle once                          # 只改最近的一批（默认行为）
retitle once --historical --dry-run   # 预览所有"装 retitle 之前"的会话
retitle once --historical             # 真把整个历史跑一遍（不受 max-age / batch 限制）
retitle once --session <id>           # 强行只改某一个会话
```
...
- **不会"装上就把你历史聊天全改了"。** 首次运行会记一个基线时间戳，
  ...
  （或跑 `retitle once --historical`）的主动行为，不是装守护进程的副作用。
...
**装上 retitle 之后，我之前那些旧聊天会怎么样?**
默认什么都不会发生。首次运行会记一个基线时间戳，后台只会改"基线之后才活跃"
的会话。想把积压的历史也跑一遍？点 dashboard 上的 **"改名历史会话"**，
或者跑 `retitle once --historical --dry-run` 先预览，没问题再去掉 `--dry-run`。
