...
machine. Mirrors the Swift app's RenameCLI.swift so behavior is consistent
across platforms."""
...
def find_rename() -> str | None:
    """Locate the ``rename`` binary. Honours $RENAME_BIN, then $PATH, then
    falls back to common install locations."""
    env = os.environ.get("RENAME_BIN")
    if env and Path(env).exists():
        return env
    via_path = shutil.which("rename")
    if via_path:
        return via_path
    candidates = [
        Path.home() / ".local/bin/rename",
        Path("/opt/homebrew/bin/rename"),
        Path("/usr/local/bin/rename"),
    ]
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        if appdata:
            candidates.append(
                Path(appdata) / r"Python\PythonNN\Scripts\rename.exe"
            )  # placeholder; user typically installs via pipx
        candidates.extend(
            [
                Path.home() / ".local/bin/rename.exe",
                Path.home() / ".local/Programs/rename/rename.exe",
                Path(r"C:\Program Files\rename\rename.exe"),
            ]
        )
    for c in candidates:
        if c.exists():
...
class RenameCLI:
    ...
    def _run(self, args: list[str], timeout: int = 90) -> bytes:
        try:
            ...
            raise RenameError(f"rename binary missing: {e}") from e
        except subprocess.TimeoutExpired as e:
            raise RenameError(f"rename {args[0]} timed out") from e
        if res.returncode != 0:
            stderr = res.stderr.decode("utf-8", errors="replace").strip()
            raise RenameError(f"rename {args[0]} failed: {stderr or res.returncode}")
        return res.stdout
    ...
    def rename_historical(self, dry_run: bool = False) -> str:
        ...
        try:
            ...
            raise RenameError("rename once --historical timed out") from e
        stderr = res.stderr.decode("utf-8", errors="replace")
        if res.returncode != 0:
            raise RenameError(
                f"rename once --historical failed: "
...
class DaemonControl:
    """Platform-specific daemon start/stop. On Windows rename has no service
    ...
    def is_running(self, status: dict | None) -> bool:
        # Prefer the daemon status reported by rename (covers launchd/systemd).
    ...
    @staticmethod
    def _plist_path() -> str:
        return str(Path.home() / "Library/LaunchAgents/com.github.rename.plist")
